...
public:
  ...
  HS071_NLP(std::vector< std::vector<int> > &OccurenceArray, std::set<int> &AllVariable,  int Num_of_Variables,
            int Num_of_VariablesEachConst, int Num_of_Const, int *Answer){
      this->OccurenceArray = OccurenceArray;
      this->AllVariable = AllVariable;
      this->Num_of_Variables = Num_of_Variables;
      this->Num_of_VariablesEachConst = Num_of_VariablesEachConst;
      this->Num_of_Const = Num_of_Const;
      
      this->equationArray = (int*)malloc(sizeof(int)* Num_of_Const * Num_of_Variables);
    
      //zero equationArray
      for(int i=0;i<Num_of_Const;i++){
          for(int j=0;j<Num_of_Variables;j++){
              this->equationArray[i*Num_of_Variables + j] = 0;
          }
      }
    
      //insert valid bit into equation array;
      for(int i=0;i<OccurenceArray.size();i++){
          for(int j=0;j<OccurenceArray[i].size();j++){
              this->equationArray[i*Num_of_Variables + OccurenceArray[i][j]] = 1;
          }
      }
      mAnswer = Answer;
      *mAnswer = 0;
